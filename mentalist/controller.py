...
from . import model
from . import view
from .tools.break_to_wordlist import build_wordlist
from .tools.site_to_wordlist import fetch_and_build
...
class UrlFirstNamesDialog(Tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Site Converter")
        ...
        Tk.Label(frm, text="Enter the URL to fetch (include http/https):").pack(anchor="w")
        url_entry = Tk.Entry(frm, textvariable=self.var_url, width=52)
        url_entry.pack(fill="x", pady=(2, 8))
        Tk.Checkbutton(frm, text="Filter by First Names?", variable=self.var_filter).pack(anchor="w", pady=(0, 8))

        btns = Tk.Frame(frm)
        btns.pack(fill="x")
        Tk.Button(btns, text="Cancel", command=self._on_cancel).pack(side="right")
        Tk.Button(btns, text="OK", command=self._on_ok, default="active").pack(side="right", padx=(0, 6))
    ...
    def _on_ok(self):
        ...
        if not url:
            tkinter.messagebox.showwarning("Site Converter", "Please enter a URL.", parent=self)
            return
        self.result = (url, self.var_filter.get())
        self.destroy()
...
class Controller():
    ...
    # -----------------------
    # Tools → Break text → Wordlist
    # -----------------------
    def on_break_to_wordlist(self, *args):
        """Assistente para quebrar texto em wordlist:
        1) escolher ficheiro (ou pasta)
        2) opções (remover pontuação / minúsculas)
        3) escolher caminho de saída
        4) gerar e oferecer abrir ficheiro
        """
        master = self.mainview.master

        # 1) escolher ficheiro; se cancelar, perguntar pasta
        file_path = tkinter.filedialog.askopenfilename(parent=master, title="Escolher ficheiro de texto")
        if not file_path:
            dir_path = tkinter.filedialog.askdirectory(parent=master, title="Escolher pasta com ficheiros de texto")
            if not dir_path:
                return
    ...
    (
        ...
        "[Yes] = remove + lowercase  |  [No] = keep as is\n\n"
        "Next, choose where the processed text (one word per line) will be saved."
    ),
        parent=master
)
        ...
        if reply is None:
            return
        remove_punct = bool(reply)
        to_lower = bool(reply)


        default_out = str(Path.home() / "wordlist.txt")
        out_name = tkinter.filedialog.asksaveasfilename(
            parent=master, title="Salvar wordlist como",
        ...
        try:
            count = build_wordlist(input_path, out_path, remove_punct=remove_punct, to_lower=to_lower)
        except Exception as e:
            tkinter.messagebox.showerror("Erro", f"Falhou gerar wordlist:\n{e}", parent=master)
            return

        open_reply = tkinter.messagebox.askyesno(
            "Wordlist criada",
            f"{count} palavras escritas em:\n{out_path}\n\nAbrir para importar em Base Words → Custom File?",
            parent=master
        )
        if open_reply:
            try:
                ...
                tkinter.messagebox.showinfo("Info", "Abra o ficheiro manualmente: Base Words → Custom File.", parent=master)

    def on_site_to_wordlist(self, *args):
        """
        Tools → Site Converter in Wordlist:
        Ask for URL, ask options, ask output path, fetch + build wordlist.
        """
        master = self.mainview.master

        dlg = UrlFirstNamesDialog(master)
        master.wait_window(dlg)
        if not dlg.result:
            return
        url, filter_first_names = dlg.result
        ...
        reply = tkinter.messagebox.askyesnocancel(
            ...
            (
                "Remove punctuation and convert to lowercase?\n"
                "[Yes] = remove + lowercase  |  [No] = keep as is\n\n"
                "Next, choose where to save the resulting wordlist (one word per line)."
            ),
            parent=master
        )
        if reply is None:
        ...
        default_out = str(Path.home() / "site_wordlist.txt")
        out_name = tkinter.filedialog.asksaveasfilename(
            parent=master,
        ...
        try:
            count = fetch_and_build(
                url,
                out_path,
                remove_punct=remove_punct,
                to_lower=to_lower,
                filter_first_names=filter_first_names
            )
        except Exception as e:
            tkinter.messagebox.showerror("Error", f"Failed to build wordlist from URL:\n{e}", parent=master)
            return

        open_reply = tkinter.messagebox.askyesno(
            "Wordlist created",
            f"{count} words written to:\n{out_path}\n\nOpen now?",
            parent=master
        )
        if open_reply:
            try:
                ...
                tkinter.messagebox.showinfo("Info", "Please open the file manually.", parent=master)
    ...
    def remove_attr(self, node_view, attr_view):
        ...
        '''
        node_idx = self.mainview.nodes.index(node_view)
        self.model.nodes[node_idx].attrs[node_view.attrs.index(attr_view)].stop_calculating()

        del self.model.nodes[node_idx].attrs[node_view.attrs.index(attr_view)]
