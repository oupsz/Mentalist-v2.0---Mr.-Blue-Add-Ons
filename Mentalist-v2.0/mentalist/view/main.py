...
# ADD (perto dos demais imports Tk)
...
class MainWindow(Tk.Frame):
    ...
    def __init__(self, controller, master=None, width=730, height=800):
        ...
        filemenu.add_command(label='Quit', command=self.controller.exit)
        ...
        # Tools menu items (only here)
        toolsmenu.add_command(label='Break text -> Wordlist', command=self.controller.on_break_to_wordlist)
        toolsmenu.add_command(label='Site Converter in Wordlist', command=self.controller.on_site_to_wordlist)

        # global keyboard shortcut
        self.master.bind_all(
            '<' + cmd_key + 'Shift-w>',
            lambda e: self.after(100, self.controller.on_break_to_wordlist)
        )
        self.master.bind_all(
            '<' + cmd_key + 'Shift-u>',
            lambda e: self.after(100, self.controller.on_site_to_wordlist)
        )

        print("[MENTALIST] Tools menu added")
        def pcommand():
            self.on_process(type_='full')  # after() doesn't like partial()
        ...
        def rcommand():
            self.on_process(type_='hashcat')
        processmenu.add_command(
            label='Hashcat/John Rules',
            command=rcommand,
...
def word_count_to_string(words):
    ...
    if isinstance(words, int):
	        words = locale.format_string("%d", words, grouping=True)  # add commasnd
    return words
